...
class AdminResultsService {
  ...
  async getChallengeStatistics(eventId) {
    const response = await apiClient.get(`/admin/results/challenges/${eventId}`);
    return response.data;
  }
  ...
  async getLeaderboard(eventId) {
    const response = await apiClient.get(`/admin/results/leaderboard/${eventId}`);
    return response.data;
  }
  ...
  async downloadExport(eventId = '') {
    const url = eventId ? `/admin/results/export/${eventId}` : '/admin/results/export';
