...
const LoginPage = () => {
  ...
  const { login, isParticipantAuthenticated, loading: authLoading } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    if (!authLoading && isParticipantAuthenticated) {
      navigate('/participant/dashboard', { replace: true });
    }
  }, [authLoading, isParticipantAuthenticated, navigate]);
    ...
    <div className="min-h-screen cinematic-bg text-slate-100 flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-cyan-600 selection:text-white">
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col items-center">
        ...
        <p className="text-[10px] font-mono text-slate-600 text-center mt-5">
          Developed by Cout Masters Coding Club &copy; 2026. All rights reserved.
        </p>
      </div>
    </div>
  );
