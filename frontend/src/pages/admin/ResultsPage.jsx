  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        setError(err.message || 'Unable to load events. Please refresh and try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <AdminLayout title="Results & Statistics">
        <div className="text-slate-400 p-8 font-mono">Loading Results &amp; Statistics...</div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout title="Results & Statistics">
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        {selectedEventId && selectedEventInfo && (
          <div className={`p-4 border rounded-xl font-mono text-sm flex items-center justify-center gap-3 ${
            selectedEventInfo.status === 'LIVE' ? 'bg-emerald-900/20 border-emerald-800 text-emerald-400' :
            selectedEventInfo.status === 'UPCOMING' ? 'bg-amber-900/20 border-amber-800 text-amber-400' :
            ...
            {selectedEventInfo.status === 'UPCOMING' && 'Event has not started yet.'}
            {selectedEventInfo.status === 'COMPLETED' && 'Event completed. Showing final recorded execution data.'}
          </div>
        )}
        ...
        {!selectedEventId ? (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center text-slate-400 font-mono">
            {events.length === 0 ? 'NO EVENTS AVAILABLE' : 'Select an event from the dropdown above to view participant results and statistics.'}
          </div>
        ) : (
          <>
            {eventDataLoading ? (
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
                <div className="text-slate-400 font-mono mb-2 font-bold text-lg">NO ACTIVITY YET</div>
                <div className="text-slate-500">Participants have not executed any challenges for this event.</div>
              </div>
            ) : (
              <div className="space-y-8">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                  <div className="p-5 border-b border-slate-800">
                    <h2 className="text-lg font-bold text-slate-200 font-mono">Recent Activity</h2>
                  </div>
                          ...
                          recentAttempts.map(attempt => (
                            <tr key={attempt._id} className="hover:bg-slate-800/30">
                              ...
                              <td className="px-4 py-3">
                                <div className="font-medium text-slate-200">{attempt.participant?.name || 'Unknown'}</div>
                              </td>
                              <td className="px-4 py-3 text-cyan-400">
                                {attempt.challenge?.title || `Challenge ${attempt.challenge?.challengeNumber || 'Unknown'}`}
                              </td>
                              <td className="px-4 py-3">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                                  attempt.status === 'SUCCESS' ? 'bg-emerald-900/50 text-emerald-400' : 'bg-rose-900/50 text-rose-400'
                                }`}>
                                  {attempt.status}
                                </span>
                              </td>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                  <div className="p-5 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <h2 className="text-lg font-bold text-slate-200 font-mono">Participant Performance</h2>
                        ...
                        {participantResults.length === 0 ? (
                          ...
                          participantResults.map(p => (
                            <tr key={p._id} className="hover:bg-slate-800/30">
                              ...
                              <td className="px-4 py-3 text-center font-bold text-emerald-400">{p.successfulExecutions}</td>
                              <td className="px-4 py-3 text-center font-bold text-rose-400">{p.failedExecutions}</td>
                              <td className="px-4 py-3 text-right text-xs text-slate-400">
                                {p.lastActivity ? new Date(p.lastActivity).toLocaleTimeString() : 'N/A'}
                              </td>
                            </tr>
                          ))
                        )}
                  ...
                  <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {challengeStats.length === 0 ? (
                      ...
                      challengeStats.map(c => (
                        <div key={c._id} className="bg-slate-950 border border-slate-800 rounded-lg p-4 font-mono">
                          <h3 className="font-bold text-slate-200 mb-3 truncate" title={c.title}>{c.title}</h3>
                          {c.attemptsCount === 0 ? (
                            <div className="text-slate-500 text-sm">No attempts yet</div>
                          ) : (
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                <span className="text-slate-400">Attempts:</span>
                                <span className="text-slate-200">{c.attemptsCount}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-slate-400">Success:</span>
                                <span className="text-emerald-400 font-bold">{c.successfulExecutions}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-slate-400">Failed:</span>
                                <span className="text-rose-400 font-bold">{c.failedExecutions}</span>
                              </div>
                            </div>
                          )}
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50 shrink-0">
              <h3 className="text-lg font-bold text-slate-100 font-mono">Execution Detail</h3>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className="text-sm font-semibold text-slate-200">{selectedAttempt.participant?.name || 'Unknown'}</span>
                  <div className="text-xs text-slate-500">{selectedAttempt.participant?.email}</div>
                </div>
                <div>
                  <span className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1">Challenge</span>
                  <span className="text-sm font-semibold text-slate-200">{selectedAttempt.challenge?.title || 'Unknown'}</span>
                </div>
                <div>
                  <span className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1">Status</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold inline-block ${
                        selectedAttempt.status === 'SUCCESS' ? 'bg-emerald-900/50 text-emerald-400' : 'bg-rose-900/50 text-rose-400'
                      }`}>
                    {selectedAttempt.status}
                  </span>
                </div>
                <div>
                  <span className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1">Execution Time &amp; Stamp</span>
                  <span className="text-sm text-slate-300 font-mono">
                    {selectedAttempt.executionTime}ms
                  </span>
                  <div className="text-xs text-slate-500">{new Date(selectedAttempt.createdAt).toLocaleString()}</div>
                </div>
              </div>
              ...
              <div>
                ...
                <pre className={`bg-slate-950 border border-slate-800 p-3 rounded-lg text-xs font-mono whitespace-pre-wrap max-h-40 overflow-y-auto ${
                  selectedAttempt.status === 'SUCCESS' ? 'text-emerald-400' : 'text-rose-400'
                }`}>
